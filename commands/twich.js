...
module.exports = {
  ...
  async execute(message, args) {
    if (message.channel.id !== '1264361701977886730') {
    ...
    try {
      ...
      if (streamData.length > 0 && streamData[0].type === 'live') {
        message.channel.send(`https://www.twitch.tv/${username} está ao vivo!`);
      } else {
        message.channel.send(`https://www.twitch.tv/${username} não está ao vivo no momento.`);
      }
    } catch (error) {
      console.error(error);
